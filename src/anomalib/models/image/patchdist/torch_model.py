...
from .detector import KNNDetector, Detector
from .distribution import DistanceDistribution
...
class PatchDistModel(nn.Module):
    ...
    def __init__(
        ...
        self.feature_extractor = get_feature_extractor(backbone, backbone_path, layer)
    ...
    def forward(
        ...
        # extract input tensor meta data
        _, _, h_original, w_original = input_tensor.shape
        ...
        image_anomaly_map = self.anomaly_map_generator(patch_anomaly_score.to(input_device), (h_original, w_original))

        return {
            "anomaly_map": image_anomaly_map,
            "pred_score": image_anomaly_score.to(input_device),
            "patch_score": patch_anomaly_score
        }

    @staticmethod
    def reshape_embedding(embedding: torch.Tensor) -> torch.Tensor:
        """Reshape [B, C, PatchH, PatchW] → [B*PatchH*PatchW, C] for KNN matching."""
...
def load_model(model_name: str, model_path: str | Path | None, model_kind: PatchDistBackbone) -> nn.Module:
    """Load a PyTorch model given its name, path, and type.

    Note: Cannot use features only for timm, not available for vision transformers (for example)
    """
    load_fn = {
...
def get_backbone_kind(backbone: str) -> PatchDistBackbone:
    """Determine the kind of backbone from the model string prefix."""
...
def get_feature_extractor(
    ...
    """Load the backbone model and create a feature extractor for ``layer``."""
    kind = get_backbone_kind(backbone)
    model = load_model(model_name=backbone, model_path=backbone_path, model_kind=kind)
